...
(function() {
    ...
    // ================================================================
    // --- НОВАЯ ЛОГИКА: РАБОТА С LОCALSTORAGE ---
    // ================================================================
    ...
    endTripBtn.addEventListener('click',()=>{
        ...
        currentTrip=[];
        renderHistory();
        renderCurrentTrip(); // Обновляем инфо о текущем рейсе (теперь он пуст)
    ...
    endShiftBtn.addEventListener('click', async () => {
        if (shiftHistory.length === 0) {
            alert('Нельзя завершить смену без выполненных рейсов.'); return;
        }
        ...
        try {
            ...
            clearState(); // <-- ОЧИЩАЕМ СОСТОЯНИЕ ПОСЛЕ УСПЕШНОЙ ОТПРАВКИ
    ...
    // --- НАЧАЛЬНАЯ ЗАГРУЗКА ---
    // Проверяем, была ли смена активна
    if (localStorage.getItem('shiftInProgress') === 'true') {
        logToScreen("Обнаружена активная смена. Восстанавливаю состояние...");
        if (typeof ymaps === 'undefined') {
            logToScreen('КРИТИЧЕСКАЯ ОШИБКА: `ymaps` не найден.'); return;
        }
        ymaps.ready(init); // Инициализируем карты
        loadState(); // Загружаем данные из localStorage
        showMainApp(); // Показываем главный экран
    } else {
