(function() {
    ...
    function applyTheme(theme) { document.body.dataset.theme = theme; localStorage.setItem('theme', theme); const themeColor = theme === 'dark' ? '#18191a' : '#ffffff'; if (themeColorMeta) themeColorMeta.content = themeColor; }
    const savedTheme = localStorage.getItem('theme'), prefersDark = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches, initialTheme = savedTheme || (prefersDark ? 'dark' : 'light');
    applyTheme(initialTheme);

    // --- БЛОК ОТЛАДКИ ---
    const debugLog = document.getElementById('debug-log'), clearLogBtn = document.getElementById('clear-log-btn'), copyLogBtn = document.getElementById('copy-log-btn');
    let logContent = localStorage.getItem('courierAppLog') || ''; if(debugLog) debugLog.innerHTML = logContent;
    function logToScreen(message) { if (debugLog) { const time = new Date().toLocaleTimeString(); const newLog = `[${time}] ${message}\n`; debugLog.innerHTML += newLog; logContent += newLog; localStorage.setItem('courierAppLog', logContent); debugLog.scrollTop = debugLog.scrollHeight; } console.log(message); }
    ...
    // --- ЭЛЕМЕНТЫ DOM ---
    const splashScreen = document.getElementById('splash-screen'), appContainer = document.querySelector('.app-container'), screens = document.querySelectorAll('.screen');
    ...
    let authToken = localStorage.getItem('courierAuthToken'), userEmail = localStorage.getItem('courierUserEmail'), userName = localStorage.getItem('courierUserName');
    let userCoords = null, zonesGeoJSON, currentTrip = [], shiftHistory = [], mapsInitialized = false;

    // --- СЕТЕВАЯ ЛОГИКА (АДАПТИРОВАНА ДЛЯ VERCEL) ---
    async function apiFetch(action, method = 'POST', payload = {}) {
        ...
        logToScreen(`Отправляю ${method} запрос: ${action}`);
        const response = await fetch(endpoint, options);
        const responseText = await response.text();
        if (!response.ok) {
            try { const errorData = JSON.parse(responseText); throw new Error(errorData.error); }
            catch (e) { throw new Error(`Неизвестная ошибка сервера (статус ${response.status})`); }
        }
        if (response.status === 201 || response.status === 204 || !responseText) return null;
        return JSON.parse(responseText);
    }

    // --- УПРАВЛЕНИЕ ЭКРАНАМИ ---
    function showScreen(screenId) { logToScreen(`Переключаюсь на экран: ${screenId}`); screens.forEach(s => s.classList.add('hidden')); document.getElementById(screenId)?.classList.remove('hidden'); if (screenId !== 'auth-screen' && screenId !== 'update-name-screen') localStorage.setItem('lastActiveScreen', screenId); }
    ...
    loginBtn.addEventListener('click', async () => {
        try {
            const data = await apiFetch('login', 'POST', { email: loginEmailInput.value, password: loginPasswordInput.value });
            authToken = data.token; userEmail = data.email; userName = data.name;
            localStorage.setItem('courierAuthToken', authToken); localStorage.setItem('courierUserEmail', userEmail); localStorage.setItem('courierUserName', userName);
            initializeApp();
        } catch (error) { alert(`Ошибка входа: ${error.message}`); logToScreen(`Ошибка входа: ${error.message}`);}
    });
    registerBtn.addEventListener('click', async () => {
        try {
            await apiFetch('register', 'POST', { name: registerNameInput.value, email: registerEmailInput.value, password: registerPasswordInput.value });
            alert('Регистрация прошла успешно! Теперь вы можете войти.'); showLoginLink.click();
    ...
    logoutBtn.addEventListener('click', () => {
        ...
        localStorage.removeItem('courierAuthToken'); localStorage.removeItem('courierUserEmail'); localStorage.removeItem('courierUserName'); localStorage.removeItem('lastActiveScreen');
        // Важно: не удаляем shiftInProgress и данные смены
        sessionStorage.removeItem('splashShown'); showScreen('auth-screen'); logToScreen('Пользователь вышел. Данные смены сохранены локально.');
    });
    saveNameBtn.addEventListener('click', async () => {
        const newName = updateNameInput.value.trim(); if (!newName) { alert('Имя не может быть пустым'); return; }
        try {
            await apiFetch('update_name', 'POST', { name: newName });
            userName = newName; localStorage.setItem('courierUserName', userName); alert('Имя успешно сохранено!'); initializeApp();
        } catch (error) { alert(`Ошибка сохранения имени: ${error.message}`); logToScreen(`Ошибка сохранения имени: ${error.message}`); }
    });

    // --- ЛОГИКА ТЕМЫ ---
    globalThemeToggle.addEventListener('click', () => { const newTheme = document.body.dataset.theme === 'dark' ? 'light' : 'dark'; applyTheme(newTheme); logToScreen(`Тема переключена на: ${newTheme}`); });

    // --- ЛОКАЛЬНОЕ ХРАНИЛИЩЕ ---
    function saveState() { if (!userEmail) return; const state = { currentTrip, shiftHistory }; localStorage.setItem(`courierAppState_${userEmail}`, JSON.stringify(state)); logToScreen(`Состояние для ${userEmail} сохранено.`); }
    function loadState() { if (!userEmail) return; const savedState = localStorage.getItem(`courierAppState_${userEmail}`); if (savedState) { logToScreen(`Найдено состояние для ${userEmail}.`); const state = JSON.parse(savedState); currentTrip = state.currentTrip || []; shiftHistory = state.shiftHistory || []; } }
    function clearLocalShiftData() { if (userEmail) localStorage.removeItem(`courierAppState_${userEmail}`); localStorage.removeItem('shiftInProgress'); logToScreen(`Локальные данные смены для ${userEmail} очищены.`); }

    // --- ОСНОВНАЯ ЛОГИКА ПРИЛОЖЕНИЯ ---
    function initMapsAndLogic() {
        if (mapsInitialized) return; if (typeof ymaps === 'undefined') { logToScreen('ОШИБКА: `ymaps` не найден.'); return; }
        ymaps.ready(async () => {
            logToScreen("API Карт готово."); mapsInitialized = true;
            try {
                await loadZones(); const location = await ymaps.geolocation.get({ provider: 'browser' });
                userCoords = location.geoObjects.get(0).geometry.getCoordinates(); logToScreen(`Местоположение определено.`);
            } catch (e) { logToScreen(`Ошибка геолокации или загрузки зон: ${e.message}`); }
        });
    }
    ...
    addressInput.addEventListener('input', async () => {
        const text = addressInput.value; if (text.length < 3) { suggestContainer.innerHTML = ''; return; }
        try {
            const payload = userCoords ? { text: text, lat: userCoords[0], lon: userCoords[1] } : { text: text };
            const suggestions = await apiFetch('suggest', 'GET', payload);
            renderOurOwnSuggestions(suggestions);
        } catch (e) { logToScreen(`Ошибка при получении подсказок: ${e.message}`); }
    });

    function renderOurOwnSuggestions(items) {
        suggestContainer.innerHTML = ''; if (!items || items.length === 0) return;
    ...
    // --- ОБРАБОТЧИКИ КНОПОК ---
    openShiftBtn.addEventListener('click', () => { clearLocalShiftData(); localStorage.setItem('shiftInProgress', 'true'); currentTrip = []; shiftHistory = []; renderCurrentTrip(); renderHistory(); updateShiftState(true); showScreen('main-app'); });
    
    async function fetchAndRenderFullHistory() {
        try {
            logToScreen('Запрашиваю историю всех смен...');
            const shifts = await apiFetch('get_shifts', 'POST'); // Используем POST для единообразия
            ...
            if (shifts.length === 0) {
                ...
                shifts.forEach(shift => {
                    ...
                    shift.trips.reverse().forEach((trip, index) => {
                        const tripDetails = document.createElement('details'); tripDetails.style.marginLeft = "15px";
                        ...
                        trip.orders.forEach(order => {
                            const orderLi = document.createElement('li'); orderLi.style.marginLeft = "20px"; orderLi.style.background = "var(--list-item-hover-bg)";
                            orderLi.textContent = `${order.address} (${order.price} ₽)`;
                            ordersList.appendChild(orderLi);
                        });
                        tripDetails.appendChild(tripSummary); tripDetails.appendChild(ordersList); tripsList.appendChild(tripDetails);
                    });
                    shiftDetails.appendChild(shiftSummary); shiftDetails.appendChild(tripsList); fullHistoryList.appendChild(shiftDetails);
                });
            }
        } catch (error) { alert(`Не удалось загрузить историю: ${error.message}`); logToScreen(`Ошибка загрузки истории: ${error.message}`); }
    }
    ...
    clearHistoryBtn.addEventListener('click', async () => {
        if (confirm('ВЫ УВЕРЕНЫ? Это действие удалит ВСЮ вашу историю смен без возможности восстановления.')) {
            try {
                await apiFetch('delete_shifts', 'POST'); // Используем POST
                alert('История успешно очищена.'); fetchAndRenderFullHistory();
            } catch (error) { alert(`Не удалось очистить историю: ${error.message}`); logToScreen(`Ошибка очистки истории: ${error.message}`); }
        }
    });
    ...
    endShiftBtn.addEventListener('click', async () => {
        if (currentTrip.length > 0) { shiftHistory.unshift({orders:[...currentTrip]}); currentTrip = []; }
        if (shiftHistory.length === 0) {
            if (confirm('Смена пуста. Вы уверены, что хотите ее закрыть (она не будет сохранена)?')) {
                clearLocalShiftData(); showScreen('start-shift-screen');
            }
            return;
        }
        ...
        try {
            await apiFetch('save_shift', 'POST', shiftData);
    ...
    goToStartScreenBtn.addEventListener('click', () => { shiftSummarySection.classList.add('hidden'); showScreen('start-shift-screen'); });
    ...
    function initializeApp() {
        if (authToken && userEmail) {
            ...
            if (lastScreen === 'history-screen') { fetchAndRenderFullHistory(); showScreen('history-screen'); }
            else if (localStorage.getItem('shiftInProgress') === 'true') {
                loadState(); renderCurrentTrip(); renderHistory(); updateShiftState(true);
                if(currentTrip.length > 0) { currentTripSection.classList.remove('hidden'); newTripBtn.disabled = true; }
                showScreen('main-app');
            } else { showScreen('start-shift-screen'); }
        } else { showScreen('auth-screen'); }
    }

    if (sessionStorage.getItem('splashShown')) {
        splashScreen.style.display = 'none'; appContainer.classList.remove('hidden'); initializeApp();
    } else {
        setTimeout(() => { splashScreen.classList.add('hidden'); appContainer.classList.remove('hidden'); sessionStorage.setItem('splashShown', 'true'); initializeApp(); }, 3000);
    }
})();
