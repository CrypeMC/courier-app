...
async function handler(event) {
    if (event.path.includes('/suggest')) {
        // --- Логика для подсказок (без изменений) ---
        try {
            if (!suggestApiKey) { throw new Error("YANDEX_SUGGEST_API_KEY не установлен на Netlify!"); }
            ...
            if (response.status !== 200) { throw new Error(`Яндекс вернул ошибку: ${data.message || 'Неизвестная ошибка'}`); }
        ...
        try {
            // ПРОВЕРКА: есть ли у нас строка подключения?
            if (!mongoUri) {
                throw new Error("MONGODB_URI не установлен на Netlify!");
            }
            ...
            // Мы возвращаем клиенту точный текст ошибки от MongoDB.
            console.error('Функция базы данных сломалась:', error);
            return { 
                statusCode: 500, 
                body: JSON.stringify({ error: `Ошибка MongoDB: ${error.message}` })
            };
        } finally {
            await client.close();
        }
